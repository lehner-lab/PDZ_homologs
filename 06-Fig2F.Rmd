...
correlation_test<-cor.test(single_dG_Salinas_2018_pdz2_long_wddG$Salinas_2018_single_dG, single_dG_Salinas_2018_pdz2_long_wddG$ddg, use = "complete.obs")


R_val <- correlation_test$estimate
p_val <- correlation_test$p.value
N_val <- sum(!is.na(single_dG_Salinas_2018_pdz2_long_wddG$ddg) &
             !is.na(single_dG_Salinas_2018_pdz2_long_wddG$Salinas_2018_single_dG))
...
ggplot(single_dG_Salinas_2018_pdz2_long_wddG, aes(x = Salinas_2018_single_dG, y = ddg)) +
  ...
  annotate("text", x = -1.4, y = 2.7, hjust = 0, vjust = 1,
           label = sprintf("r = %.2f\np = %.2e\nn = %d", R_val, p_val, N_val)) +
  theme_classic() +
  ylab("∆∆Gb doubledeepPCA") +
  ggtitle("PSD95-PDZ2 | NMDAR2A")

ggsave(paste0("Figs/Fig2/Fig2Fvalidations_salinas_pdz2_nmdar2a.pdf"), width=3, height=3)
...
correlation_test<-cor.test(single_dG_Salinas_2018_pdz3_long_wddG$Salinas_2018_single_dG, single_dG_Salinas_2018_pdz2_long_wddG$ddg, use = "complete.obs")
R_val <- correlation_test$estimate
p_val <- correlation_test$p.value
N_val <- sum(!is.na(single_dG_Salinas_2018_pdz3_long_wddG$ddg) &
             !is.na(single_dG_Salinas_2018_pdz3_long_wddG$Salinas_2018_single_dG))

ggplot(single_dG_Salinas_2018_pdz3_long_wddG, aes(x = Salinas_2018_single_dG, y = ddg)) +
  geom_point(alpha = 0.5, size = 2) +
  geom_errorbar(aes(ymin = ddg - std_ddg, ymax = ddg + std_ddg), width = 0.1, alpha = 0.3) +
  stat_poly_line(color = "green3", fill = "green", alpha = 0.5) +
  annotate("text", x = -1.3, y = 3.4, hjust = 0, vjust = 1,
           label = sprintf("r = %.2f\np = %.2e\nn = %d", R_val, p_val, N_val)) +
  theme_classic() +
  ylab("∆∆Gb doubledeepPCA") +
  ggtitle("PSD95-PDZ3 | CRIPT")

ggsave(paste0("Figs/Fig2/Fig2Fvalidations_salinas_pdz3_cript.pdf"), width=3, height=3)
