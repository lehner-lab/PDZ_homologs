...
all_ddg_table_binding$library_name<-gsub("_", "-", gsub(" binding ", "\n", (unlist(lapply(all_ddg_table_binding$library, lib_code_to_name, libraries_names, libraries_names_plots)))))


ggplot(all_ddg_table_binding[is.na(binding_interface_contacts)], aes(x=ddg))+#!structural_alignment_pos %in% all_bi], aes(x=ddg))+
  theme_classic()+
...
ggsave(paste0("Figs/Fig4/Fig4Addg_distributions.pdf"), width=4, height=3.5)
```
...
library(ggh4x)
all_allo_table$library_name<-gsub("_", "-", gsub(" binding ", " | ", (unlist(lapply(all_allo_table$library, lib_code_to_name, libraries_names, libraries_names_plots)))))
curve_coefficients$library_name<-gsub("_", "-", gsub(" binding ", " | ", (unlist(lapply(curve_coefficients$library, lib_code_to_name, libraries_names, libraries_names_plots)))))
...
p
ggsave(paste0("Figs/Fig4/Fig4Bdecay_curves_pdz.pdf"), width=14, height=3)
...
```{r}
n_plot<-cor_matrix_melted[, .(.N), by=category_comparison]
ggplot(cor_matrix_melted, aes(x=factor(category_comparison, levels=c("same_pdz", "same_lig", "class2", "unrelated"),labels=c("same\npdz", "same\nligand","class1\nvs\nclass2", "not\nrelated")), y=value, color=category_comparison, fill=category_comparison))+
...
theme(axis.text.x=element_text(size=13,face="bold", color="black"), 
        ...
        axis.title.y=element_text(size=13,face="bold", color="black"),
        axis.title.x=element_text(size=13,face="bold", color="black"))+
  geom_text(data=n_plot, aes(y=0.1, label=paste0("N=", N)), color="black")
...
```{r warning=FALSE}
# Ooverall correlations but also the correlations of the allosteric sites
...
n_plot<-overlap_matrix_melted[, .(.N), by=category_comparison]
...
theme(axis.text.x=element_text(size=13,face="bold", color="black"), 
        axis.text.y=element_text(size=13,face="bold", color="black"),
        axis.title.y=element_text(size=13,face="bold", color="black"))+
  geom_text(data=n_plot, aes(y=0.9, label=paste0("N=", N)), color="black")
...
ggsave(paste0("Figs/Fig4/Fig4Ddecay_dstributions_examples.pdf"), width=2.1, height=2)
...
ggsave(paste0("Figs/Fig4/Fig4Ddecay_dstributions_examples2.pdf"), width=2.1, height=2.2)
...
all_ddg_table_allo[, class_plot :=
  factor(class,
         levels = 0:4,
         labels = labels_plot)]

p_dt <- rbindlist(list(
  data.table(group1 = labels_plot[2],
             group2 = labels_plot[3],
             p = wilcox.test(ddg ~ class_plot,
                             data = all_ddg_table_allo[class %in% c(1,2)])$p.value),

  data.table(group1 = labels_plot[3],
             group2 = labels_plot[4],
             p = wilcox.test(ddg ~ class_plot,
                             data = all_ddg_table_allo[class %in% c(2,3)])$p.value),

  data.table(group1 = labels_plot[3],
             group2 = labels_plot[5],
             p = wilcox.test(ddg ~ class_plot,
                             data = all_ddg_table_allo[class %in% c(2,4)])$p.value),

  data.table(group1 = labels_plot[2],
             group2 = labels_plot[5],
             p = wilcox.test(ddg ~ class_plot,
                             data = all_ddg_table_allo[class %in% c(1,4)])$p.value)
))
p_dt[, label := sprintf("p = %.2e", p)]
p_dt[, y.position := c(3.2, 3.8, 4.4, 5.0)]




n_plot<-all_ddg_table_allo[, .(.N) , by=class_plot]
ggplot(all_ddg_table_allo,aes(x=class_plot, y=ddg, fill=class_plot, color=class_plot))+
  ...
  theme(axis.text.y=element_text(size=13,face="bold", color="black"),
        axis.title.y=element_text(size=13,face="bold", color="black"))+
  geom_text(data=n_plot, aes(y=-2, label=paste0("N=", N)), size=3, color="black")+
  stat_pvalue_manual(p_dt,label = "label",y.position = "y.position",xmin = "group1",xmax = "group2",bracket.size = 0.6,tip.length = 0.01,inherit.aes = FALSE)

  

ggsave(paste0("Figs/Fig4/Fig4Ghotspots_distributions.pdf"), width=4.3, height=3)
```
...
all_ddg_table_allo$library_name<-gsub("_", "-", gsub(" binding ", " | ", (unlist(lapply(all_ddg_table_allo$library, lib_code_to_name, libraries_names, libraries_names_plots)))))
...
pvals <- as.data.frame(all_ddg_table_allo) %>%
  group_by(library_name) %>%
  group_modify(~{
    ...
    valid_comparisons <- comparisons[
      sapply(comparisons, function(comp) {
        sum(dat$class == comp[1]) > 1 &&
        sum(dat$class == comp[2]) > 1
      })
    ]
    
    if(length(valid_comparisons) == 0) return(tibble())
    
    dat %>%
      wilcox_test(ddg ~ class,
...
n_plot<-all_ddg_table_allo[, .(.N) , by=.(class, library_name)]
ggplot(all_ddg_table_allo,aes(x=factor(class, levels=c(0,1,2,3,4), labels=labels_plot), y=ddg, fill=factor(class, levels=c(0,1,2,3,4), labels=labels_plot), color=factor(class, levels=c(0,1,2,3,4))))+
  ...
  geom_text(data=n_plot, aes(y=-1.7, label=paste0( N)), size=2, color="black")

ggsave(paste0("Figs/Fig4/FigS4Ghotspots_distributions_per_lib.pdf"), width=12, height=3)
...
ggsave(paste0("Figs/Fig4/Fig4Ehotspots_per_pdz_2.pdf"), width=3, height=4)
```
...
ggplot(plot_alignment_dt, aes(x = factor(structural_alignment_pos), y = library_name, fill = median_ddg, label=WT_aa)) +
  geom_tile()+
...
ggsave(paste0("Figs/Fig4/Fig4Fheatmap_allo_hotspots.pdf"), , width=20, height=3.5)
```
...
# all allosteric positions, in all PDZs
...
chimera commands to make a side by side image comparing the sector
```{r}


#I'll do a new table with all together
all_sites<-unique(all_ddg_table$structural_alignment_pos)
all_sites_hotspots_sector_dt<-data.table(structural_alignment_pos=all_sites)

all_sites_hotspots_sector_dt[, any_bi:=structural_alignment_pos %in% unique(all_ddg_table[binding_interface_contacts==T]$structural_alignment_pos)]
all_sites_hotspots_sector_dt[, any_allo:=structural_alignment_pos %in% unique(all_ddg_table[is.na(binding_interface_contacts) & assay=="binding"]$structural_alignment_pos)]

all_sites_hotspots_sector_dt[, any_bi_hotspot:=structural_alignment_pos %in% unique(BI_hotspots_dt[significant==T]$structural_alignment_pos)]
all_sites_hotspots_sector_dt[, any_allo_hotspot:=structural_alignment_pos %in% unique(allo_hotspots_dt[allo_hotspot==T]$structural_alignment_pos)]

all_sites_hotspots_sector_dt[, bi_consv_hotspot:=structural_alignment_pos %in% unique(BI_hotspots_dt[BI_consv_hotspot==T]$structural_alignment_pos)]
all_sites_hotspots_sector_dt[, allo_consv_hotspot:=structural_alignment_pos %in% unique(allo_hotspots_dt[allo_consv_hotspot==T]$structural_alignment_pos)]

all_sites_hotspots_sector_dt[, sector:=structural_alignment_pos %in% sector_SCA.Mclaughlin2012_pdz3_aln]


all_sites_allo_dt_tmp<-left_join(all_sites_hotspots_sector_dt, all_ddg_table[library=="psd95_pdz3_cript", c("structural_alignment_pos", "Pos")])

#setnames(all_sites_allo_dt_tmp, "allo_hotspot", "all_allo_hotspot")
all_sites_allo_dt_tmp<-left_join(all_sites_allo_dt_tmp, allo_hotspots_dt[library=="psd95_pdz3_cript" , c("structural_alignment_pos", "Pos", "allo_hotspot")])

setnames(all_sites_allo_dt_tmp, "Pos", "pdz3_Pos")
setnames(all_sites_allo_dt_tmp, "allo_hotspot", "pdz3_allo_hotspot")

print("ALL SITES")
print("printing sector sites")
print(paste0("select /A:",paste0(unique(all_sites_allo_dt_tmp[sector==T]$pdz3_Pos), collapse=",")))
print(paste0("(n=",length(unique(all_sites_allo_dt_tmp[sector==T]$pdz3_Pos)), ")"))
print("printing any allo hotspots sites")
print(paste0("select /A:",paste0(unique(all_sites_allo_dt_tmp[any_allo_hotspot==T]$pdz3_Pos), collapse=",")))
print(paste0("(n=",length(unique(all_sites_allo_dt_tmp[any_allo_hotspot==T]$pdz3_Pos)), ")"))
print("printing consv allo hotspots sites")
print(paste0("select /A:",paste0(unique(all_sites_allo_dt_tmp[allo_consv_hotspot==T]$pdz3_Pos), collapse=",")))
print(paste0("(n=",length(unique(all_sites_allo_dt_tmp[allo_consv_hotspot==T]$pdz3_Pos)), ")"))
print("printing any bi hotspots sites")
print(paste0("select /A:",paste0(unique(all_sites_allo_dt_tmp[any_bi_hotspot==T]$pdz3_Pos), collapse=",")))
print(paste0("(n=",length(unique(all_sites_allo_dt_tmp[any_bi_hotspot==T]$pdz3_Pos)), ")"))
print("printing consv bi hotspots sites")
print(paste0("select /A:",paste0(unique(all_sites_allo_dt_tmp[bi_consv_hotspot==T]$pdz3_Pos), collapse=",")))
print(paste0("(n=",length(unique(all_sites_allo_dt_tmp[bi_consv_hotspot==T]$pdz3_Pos)), ")"))
print("printing consv bi hotspots AND consv ALLO AND sector sites")
print(paste0("select /A:",paste0(unique(all_sites_allo_dt_tmp[(bi_consv_hotspot==T | allo_consv_hotspot==T) & sector==T]$pdz3_Pos), collapse=",")))
print(paste0("(n=",length(unique(all_sites_allo_dt_tmp[(bi_consv_hotspot==T | allo_consv_hotspot==T) & sector==T]$pdz3_Pos)), ")"))

print("printing consv bi hotspots AND sector sites")
print(paste0("select /A:",paste0(unique(all_sites_allo_dt_tmp[(bi_consv_hotspot==T) & sector==T]$pdz3_Pos), collapse=",")))
print(paste0("(n=",length(unique(all_sites_allo_dt_tmp[(bi_consv_hotspot==T) & sector==T]$pdz3_Pos)), ")"))
print("printing consv allo hotspots AND sector sites")
print(paste0("select /A:",paste0(unique(all_sites_allo_dt_tmp[(allo_consv_hotspot==T) & sector==T]$pdz3_Pos), collapse=",")))
print(paste0("(n=",length(unique(all_sites_allo_dt_tmp[(allo_consv_hotspot==T) & sector==T]$pdz3_Pos)), ")"))

print("all the rest of residues")
print(paste0("(n=",length(unique(all_sites_allo_dt_tmp[!(bi_consv_hotspot==T | allo_consv_hotspot==T) & !sector==T]$pdz3_Pos)), ")"))
...
```{r}
# enrichment of sector in overall allostery+bining
table((all_sites_hotspots_sector_dt$bi_consv_hotspot | all_sites_hotspots_sector_dt$allo_consv_hotspot), all_sites_hotspots_sector_dt$sector)
fisher.test(table((all_sites_hotspots_sector_dt$bi_consv_hotspot | all_sites_hotspots_sector_dt$allo_consv_hotspot), all_sites_hotspots_sector_dt$sector), alternative = "greater")
...
ggsave(paste0("Figs/Fig4/Fig4Jenrichment_structures.pdf"), width=3, height=4)
...
Positions 73 and 61 are conserved hotspots, and are always in the surface, check the RSASA values
```{r}
allo_hotspots_dt_tmp[allo_consv_hotspot==T & allo_hotspot==T & structure_location=="surface"][order(structural_alignment_pos)]
```


Show the differential conservation of the core and surface quantitatively
```{r}
conserved_hotspots_<-all_ddg_table_allo[allo_hotspot==T, .N, by = .(structural_alignment_pos, pdz, rsasa)][, .(N=.N, median_rsasa=median(rsasa),mean_rsasa=mean(rsasa)), by = .(structural_alignment_pos)]

ggplot(conserved_hotspots_, aes(x=N, y=median_rsasa))+
  geom_point(shape=21, alpha=0.5, size=3.5, aes(fill=N<3, color=N<3))+#,position = position_jitter(width = 0.1, height = 0))+
  #ylim(c(0,1))+
  theme_classic()+
  geom_smooth(method = "lm", color = "black", size=1.5, se=F) +
  stat_cor(method = "pearson", label.x = 1, label.y = 0.65)+
  ylab("median RSASA when hotspot")+
  scale_fill_manual(values=c("#886ac4", "#64dd17"))+
  scale_color_manual(values=c("#886ac4", "#64dd17"))+
  theme(axis.text.x=element_text(size=13,face="bold", color="black"), 
        axis.text.y=element_text(size=13,face="bold", color="black"),
        axis.title.y=element_text(size=13,face="bold", color="black"),
        axis.title.x=element_text(size=13,face="bold", color="black"))+
...
ggsave(paste0("Figs/Fig4/Fig4M_Hotspots_vs_rsasa.pdf"), width=4, height=4)
```

boxplots for the correlations of mutation effects in surface hotspots vs core hotspots
```{r}
all_ddg_table_allo_hotspots<-left_join(all_ddg_table, allo_hotspots_dt[assay=="binding", .(id, library, allo_hotspot,allo_consv_hotspot)])[assay=="binding"]


# Keep only relevant columns, including the mutation identity
dt <- all_ddg_table_allo_hotspots[
  , .(library, structural_alignment_pos, Mut, ddg, allo_hotspot, structure_location)
]

lib_pairs <- combn(unique(dt$library), 2, simplify = FALSE)

calc_allo_corrs <- function(lib1, lib2, data) {
  d1 <- data[library == lib1]
  d2 <- data[library == lib2]
  
  # merge by position and mutation
  merged <- merge(d1, d2, by = c("structural_alignment_pos", "Mut"), suffixes = c("_1", "_2"))
  
  # sites that are allo_hotspot in ANY of the two libraries
    merged <- merged[allo_hotspot_1 & allo_hotspot_2]
  
  # subsets for surface vs core
  surf <- merged[structure_location_1 == "surface" & structure_location_2 == "surface"]
  core <- merged[structure_location_1 == "core"  & structure_location_2 == "core"]
  
  cor_surface <- if (nrow(surf) > 3)
    cor(surf$ddg_1, surf$ddg_2, use = "pairwise.complete.obs", method = "pearson") else NA_real_
  
  cor_core <- if (nrow(core) > 3)
    cor(core$ddg_1, core$ddg_2, use = "pairwise.complete.obs", method = "pearson") else NA_real_
  
  data.table(lib1 = lib1, lib2 = lib2,
...
results_list <- lapply(lib_pairs, \(pair) calc_allo_corrs(pair[1], pair[2], dt))
results_dt <- rbindlist(results_list, fill = TRUE)

results_long <- melt(results_dt,
                     id.vars = c("lib1", "lib2"),
                     measure.vars = c("cor_surface", "cor_core"),
                     variable.name = "region",
                     value.name = "correlation")[,
                     region := fifelse(region == "cor_surface", "surface", "core")]


results_long2 <- results_long[
  {
    pdz1 <- tstrsplit(lib1, "_")[[1]]
    pdz2 <- tstrsplit(lib2, "_")[[1]]
    dom1 <- paste(tstrsplit(lib1, "_")[[1]], tstrsplit(lib1, "_")[[2]], sep = "_")
    dom2 <- paste(tstrsplit(lib2, "_")[[1]], tstrsplit(lib2, "_")[[2]], sep = "_")
    dom1 != dom2
  }
]

# Split lib1 and lib2 into their components: gene, pdz, ligand
results_long2[, c("gene1", "pdz1", "ligand1") := tstrsplit(lib1, "_", fixed = TRUE)]
results_long2[, c("gene2", "pdz2", "ligand2") := tstrsplit(lib2, "_", fixed = TRUE)]

# Combine gene + pdz into a single PDZ identifier
results_long2[, PDZ1 := paste(gene1, pdz1, sep = "_")]
results_long2[, PDZ2 := paste(gene2, pdz2, sep = "_")]
results_long2[, class := "unrelated"]
results_long2[PDZ1==PDZ2, class := "same_pdz"]
results_long2[ligand1==ligand2, class := "same_ligand"]


library(ggplot2)
n_plot<-results_long2[!is.na(correlation) & class=="unrelated"][, .(.N) , by=.(region)]
ggplot(results_long2[!is.na(correlation) & class=="unrelated"], aes(x = region, y = correlation, fill = region)) +
  ...
  scale_fill_manual(values = c(core = "salmon", surface = "turquoise"))+
  ...
  theme(axis.text.x=element_text(size=13,face="bold", color="black"), 
        ...
        axis.title.y=element_text(size=13,face="bold", color="black"),
        axis.title.x=element_blank())+
  geom_text(data=n_plot, aes(y=-0.4, label=paste0( "N=",N)), size=6, color="black")

ggsave(paste0("Figs/Fig4/FigS4M_hotspots_cor_surface_core.pdf"), width=4, height=4)

summary(results_long2[!is.na(correlation) & class=="unrelated" & region=="surface"]$correlation)
summary(results_long2[!is.na(correlation) & class=="unrelated" & region=="core"]$correlation)
...
The most functionally conserved allosteric hotspots are also more conserved in sequence and structure. 
...
n_plot<-allo_hotspots_dt_short_short[, .(.N) , by=.(hotspot_type_plot)]
ggplot(allo_hotspots_dt_short_short, aes(x=factor(hotspot_type_plot, levels=categories, labels=categories_names), y=consv, fill=hotspot_type_plot, color=hotspot_type_plot))+
  ...
  theme(axis.text.y=element_text(size=10,face="bold", color="black"),
         axis.title.y=element_text(size=10,face="bold", color="black"))+
  geom_text(data=n_plot, aes(y=-0.4, label=paste0("N=", N)), size=3, color="black")

ggsave(paste0("Figs/Fig4/Fig4Nsequence_conservation.pdf"), width=2, height=2.3)
```
...
n_plot<-rmsd_results[, .(.N) , by=.(variable)]
ggplot(rmsd_results, aes(x=factor(variable, levels=categories, labels=categories_names), y=value, fill=variable, color=variable))+
  ...
  theme_classic()+theme(legend.position="none",
                        ...
                        axis.text.x=element_blank(),
                        axis.ticks.x=element_blank())+
  ylim(c(-0.2,8.5))+ylab("RMSD")+
  theme(axis.text.y=element_text(size=10,face="bold", color="black"),
         axis.title.y=element_text(size=10,face="bold", color="black"))+
  geom_text(data=n_plot, aes(y=-0.2, label=paste0( "N=",N)), size=3, color="black")

ggsave(paste0("Figs/Fig4/Fig4Ostructure_conservation.pdf"), width=1.7, height=2.2)
...
n_plot<-allo_hotspots_dt_short[, .(.N) , by=.(hotspot_type_plot)]
ggplot(allo_hotspots_dt_short, aes(x=factor(hotspot_type_plot, levels=categories, labels=categories_names), y=rsasa, fill=hotspot_type_plot, color=hotspot_type_plot))+
  ...
  geom_boxplot(alpha=1, width=0.2, color="black", fill="white", outliers=F)+
  theme_classic()+
  stat_compare_means(comparisons=list(c("non-Hotspot", "Hotspot in\n1 or 2 PDZs"), c("Hotspot in\n1 or 2 PDZs", "Hotspots\nin >2/5 PDZs")), size=7, aes(label=after_stat(p)), method="wilcox.test")+
  scale_fill_manual("",values=c(  "#886ac4","grey50", "#64dd17"))+
  scale_color_manual("",values=c(  "#886ac4","grey50", "#64dd17"))+
  theme(legend.position="none",
        axis.title.x=element_blank(),
  ...
  theme(axis.text.y=element_text(size=10,face="bold", color="black"),
         axis.title.y=element_text(size=10,face="bold", color="black"))+
  geom_text(data=n_plot, aes(y=-0.1, label=paste0("N=", N)), size=4, color="black")

ggsave(paste0("Figs/Fig4/Fig4Krsasa_allo_hotspots.pdf"), width=1.7, height=2.2)
```
...
cor_res <- cor.test(
  allo_hotspots_dt_762_variableH$ddg.psd95_pdz2_cript,
  allo_hotspots_dt_762_variableH$ddg.psd95_pdz2_nmdar2a,
  use = "complete.obs",
  method = "pearson"
)
...
label_text <- sprintf("R = %.2f, p = %.2e", R_val, p_val)
...
ggplot(allo_hotspots_dt_762_variableH, aes(x=ddg.psd95_pdz2_cript, y=ddg.psd95_pdz2_nmdar2a, color=factor(structural_alignment_pos)))+
  ...
  annotate("text",
         x = min(allo_hotspots_dt_762_variableH$ddg.psd95_pdz2_cript, na.rm = TRUE),
         y = max(allo_hotspots_dt_762_variableH$ddg.psd95_pdz2_nmdar2a, na.rm = TRUE),
         hjust = 0,
         vjust = 1,
         label = label_text,
         size = 3.5)
...
cor_res <- cor.test(
  allo_hotspots_dt_771_variableH$ddg.nherf3_pdz1_dap_1,
  allo_hotspots_dt_771_variableH$ddg.nherf3_pdz1_uhmk1,
  use = "complete.obs",
  method = "pearson"
)
...
ggsave(paste0("Figs/Fig4/FigS4J_2_scatter_variable_762.png"), width=3, height=3)
```
...
n_plot<-correlations_allo_datatable[, .(.N) , by=.(class)]
ggplot(correlations_allo_datatable, aes(x=factor(class, levels=c("same_pdz", "same_lig", "unrelated"),labels=c("Same\nPDZ", "Same\nligand", "Unrelated")), y=cor, color=class, fill=class))+
  ...
  ylim(c(-0.1,1))+
  geom_text(data=n_plot, aes(y=0, label=paste0("N=", N)), size=3, color="black")
