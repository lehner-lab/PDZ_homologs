...
3. For visualization, choose alternative backgrounds as WT when there are no singles in some of the blocks.
...
p2
ggsave(paste0("Figs/Fig1/Fig1I_distributions_examples.pdf"), width=3.2, height=3.7 )
...
plot_dt <- as.data.table(
  all_fitness_modified_wide[!library %in% c("psd95_pdz3", "psd95_pdz3_cript") & !is.na(fitness_norm.1) & !is.na(fitness_norm.2)]
)
n_dt <- plot_dt[, .(N = .N), by = library_name]
...
ggplot(all_fitness_modified_wide[!library %in% c("psd95_pdz3", "psd95_pdz3_cript")], aes(x=fitness_norm.1, y=fitness_norm.2, color=modified_background))+
  ...
  stat_cor(aes(label = paste(..rr.label.., sep = "~`,`~")), label.y = 1, label.x=-2.5, color="black", size=4)+
  ...
  geom_text(
    data = n_dt,
    aes(x = -1.6, y =0.45, label = paste0("N = ", N)),
    inherit.aes = FALSE,
    size = 4
  ) +
...
ggsave(paste0("Figs/Fig1/FigS1B_normalized_overlapping.pdf"), width=3, height=8)
